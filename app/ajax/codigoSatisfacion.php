...
if(!empty($username_cliente)){?>
  <script>alert("Haga login para votar")</script><?php
	...
	$numRegistrosSatisfacion = (new SatisfacionRepositorio())->numRegistrosSatisfacion();
	$datos['ref_Satisfacionusernamerestaurantes']="Sat-" . $numRegistrosSatisfacion[0]->num_tuplas;

	$obj = (new SatisfacionRepositorio())->deleteTuplaSatisfacionUsernameNegocio($datos);
	$insertSatisfacion = (new SatisfacionRepositorio())-> insertSatisfacion($datos);
}
	//echo"Marco";
?>